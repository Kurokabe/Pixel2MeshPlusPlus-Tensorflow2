...
class MeshNetMVP2M(Model):
    ...
    def _build(self):
        with tf.compat.v1.name_scope("pixel2mesh"):
            ...
            self.layers.append(
                GraphProjection(
                    placeholders=self.placeholders, name="graph_proj_1_layer_0"
                )
            )
            self.layers.append(
                GraphConvolution(
                    input_dim=self.args.feat_dim,
            ...
            self.layers.append(
                GraphProjection(
                    placeholders=self.placeholders, name="graph_proj_2_layer_15"
                )
            )
            self.layers.append(
                GraphPooling(
                    placeholders=self.placeholders,
                    pool_id=1,
                    name="graph_pool_1to2_layer_16",
                )
            )
            self.layers.append(
                GraphConvolution(
                    input_dim=self.args.feat_dim + self.args.hidden_dim,
                    output_dim=self.args.hidden_dim,
                    gcn_block_id=2,
                    placeholders=self.placeholders,
            ...
            self.layers.append(
                GraphProjection(
                    placeholders=self.placeholders, name="graph_proj_3_layer_31"
                )
            )
            self.layers.append(
                GraphPooling(
    ...
    def build(self):
        ...
        self.merged_summary_op = tf.compat.v1.summary.merge([self.summary_loss])
